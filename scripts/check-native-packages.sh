...
if [ "$#" -ne 8 ]; then
    echo 'usage: check-native-packages.sh STATIC_CLI LICENSE DEB RPM PKGBUILD SRCINFO VERSION REPOSITORY' >&2
    exit 2
fi
...
assert_equal "$version-1" "$(dpkg-deb --field "$deb" Version)" 'deb version'
...
"$test_root/deb/usr/bin/spawnr" --version | grep -Fx "spawnr $version"
...
assert_equal "$version" "$(rpm --dbpath "$test_root/rpmdb" --query --package --queryformat '%{VERSION}' "$rpm_package")" 'rpm version'
...
"$test_root/rpm/usr/bin/spawnr" --version | grep -Fx "spawnr $version"
...
grep -F "https://github.com/$repository/releases/download/v$version/" "$srcinfo"
...
cp "$static_cli" "$test_root/aur/spawnr-$version-x86_64-linux"
